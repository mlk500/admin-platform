...
import { MediaTask, QuestionTaskTBC, Task, TaskTBC } from "../../../../redux/models/Interfaces";
...
function DuplicateTask() {
    ...
    const [description, setDescription] = useState<string>(taskToDuplicate.description || "");
    const [question, setQuestion] = useState<string>(taskToDuplicate.questionTask?.question || "");
    const [answers, setAnswers] = useState<string[]>(taskToDuplicate.questionTask?.answers || []);
    const [correctAnswer, setCorrectAnswer] = useState<number>(taskToDuplicate.questionTask?.correctAnswer ?? 0);
    const [additionalNotes, setAdditionalNotes] = useState<string>(taskToDuplicate.taskFreeTexts?.[0] || "");
    const [showMedia, setShowMedia] = useState<boolean>(!!taskToDuplicate.mediaList?.length);
    const [showQuestion, setShowQuestion] = useState<boolean>(!!taskToDuplicate.questionTask);
    const [showNotes, setShowNotes] = useState<boolean>(!!taskToDuplicate.taskFreeTexts?.length);
    const [selectedSector, setSelectedSector] = useState<number | null>(taskToDuplicate.adminIDAPI || null);
    const [withMsg, setWithMsg] = useState<boolean>(taskToDuplicate.withMsg);
    const [isLoading, setIsLoading] = useState<boolean>(false);
    const [loadingMessage, setLoadingMessage] = useState<string>("");
    const [showConfirm, setShowConfirm] = useState(false);
    const [existingMediaTasks, setExistingMediaTasks] = useState<MediaTask[]>(taskToDuplicate.mediaList || []);
    ...
    const handleDeleteMedia = (index: number) => {
        if (index < existingMediaTasks.length) {
            ...
            setNewMediaFiles(prevFiles => prevFiles.filter((_, i) => i !== newIndex));
        }
    };
    ...
    const handleSubmit = async () => {
        ...
        if (!showQuestion && !additionalNotes.trim() && newMediaFiles.length === 0 && existingMediaTasks.length === 0) {
        ...
        const sectorAdmin = sectors.find((sector) => sector.adminID === selectedSector)?.sector || "";

        const existingMediaIds = existingMediaTasks.map(media => media.mediaTaskID);
        ...
        <div className="main-container-edit-task">
            ...
            <div className="edit-task-container" dir="rtl">
                <div className="scrollable-content">
                    ...
                    <div className="input-group">
                        <label className="input-label">{DuplicateTaskHebrew.Description}</label>
                        <textarea
                    ...
                    {showMedia && (
                        <div className="input-group">
                            <label className="input-label">{DuplicateTaskHebrew.AddMedia}</label>
                            <input
                                type="file"
                                multiple
                            ...
                            <MediaViewer
                                mediaList={[...existingMediaTasks, ...newMediaFiles.map((file, index) => ({
                    ...
                    {showNotes && (
                        <div className="input-group">
                            <label className="input-label">{DuplicateTaskHebrew.AdditionalNotes}</label>
                            <textarea
                                className="task-textarea"
                    ...
                    <div className="edit-buttons">
                        ...
                        >
                            {showMedia ? DuplicateTaskHebrew.ToggleOffMedia : DuplicateTaskHebrew.ToggleOnMedia}
                        </button>
                        <button
                            type="button"
                            className="option-button"
                        ...
                        >
                            {showQuestion ? DuplicateTaskHebrew.ToggleOffQuestion : DuplicateTaskHebrew.ToggleOnQuestion}
                        </button>
                        <button
                            type="button"
                            className="option-button"
                        ...
                        >
                            {showNotes ? DuplicateTaskHebrew.ToggleOffNotes : DuplicateTaskHebrew.ToggleOnNotes}
                        </button>
                    </div>
                    <div className="input-group">
